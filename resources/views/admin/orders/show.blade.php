<div class="box box-info">
    ...
    <table class="table table-bordered">
        ...
        <tr>
            <td>{{ $item->product->name }} {{ $item->productSku->sku }}</td>
            <td>￥{{ $item->price }}</td>
            <td>{{ $item->amount }}</td>
        </tr>
        @endforeach
                ...
                <td colspan="4">
                    <form action="{{ route('admin.orders.ship', ['order' => $order->id]) }}" method="post" class="form-inline">
                        ...
                        <div class="form-group {{ $errors->has('express_no') ? 'has-error' : ''}}">
                            <label for="express_no" class="control-label">物流公司</label>
                            <input type="text" id="express_no" name="express_no" class="form-control" placeholder="输入物流单号">
